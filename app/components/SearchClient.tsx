...
export default function SearchClient({ initialPosts, emptyLabel }: SearchClientProps) {
                ...
                {filteredPosts.length === 0 ? (
                    <div className="text-center py-12">
                        <p className="text-gray-600 dark:text-gray-400 text-lg">
                            {searchTerm ? 'No posts found matching your search.' : (emptyLabel || 'No blog posts available yet.')}
                        </p>
                    </div>
                ) : (
                    filteredPosts.map((post) => {
                        const isExternal = post.slug && typeof post.slug === 'string' && post.slug.startsWith('http');
                        if (isExternal) {
                            return (
                                <a
                                    key={post.slug}
                                    ...
                                    <article className="p-6 bg-white dark:bg-background-black border border-gray-200 dark:border-gray-700 rounded-lg hover:border-gray-300 dark:hover:border-gray-600 transition-all duration-200 cursor-pointer mb-2">
                                        <div className="flex justify-between items-start gap-4">
                                            <div className="flex-1">
                                                ...
                                                <p className="text-gray-600 dark:text-gray-400 mb-4 leading-relaxed">{post.description}</p>
                                                ...
                                                <p className="text-sm text-gray-500 dark:text-gray-500">{post.readTime}</p>
                                            </div>
                                            <div className="text-right flex-shrink-0">
                                                <time className="text-sm font-medium text-gray-600 dark:text-gray-400">{formatDate(post.date)}</time>
                                            </div>
                                        </div>
                                    </article>
                                </a>
