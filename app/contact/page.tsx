    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const userMessage = errorData?.error === 'Email service not configured'
                    ? 'Email service not configured on server. Please contact me directly at himanshulodha@gmail.com.'
                    : 'Something went wrong. Please try again or email me directly at himanshulodha@gmail.com.'

                alert(userMessage)

                // Optionally show a quick mailto fallback
                const shouldOpenMail = confirm(userMessage + '\n\nOpen your mail client to send an email?')
                if (shouldOpenMail) {
                    const mailto = `mailto:himanshulodha@gmail.com?subject=${encodeURIComponent('Contact from ' + formData.name)}&body=${encodeURIComponent(formData.message)}`
